...
namespace my_container
{
    ...
    // Узел двусвязного списка выделяем в отдельный шаблон,
    // чтобы итераторы могли его видеть, не залезая внутрь List.
    template <typename T>
    struct ListNode
    {
        ListNode *prev;
    ...
    public:
        ...
        bool operator==(const ListConstIterator &rhs) const { return _p == rhs._p; }
        bool operator!=(const ListConstIterator &rhs) const { return _p != rhs._p; }

        const ListNode<T> *raw() const { return _p; }
    ...
    public:
        ...
        {
            const auto &r = static_cast<const List &>(rhs); // безопасно: одинаковые параметры шаблона
            if (_size != r._size)
            {
                return false;
            }
